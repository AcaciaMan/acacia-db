"""
Script to analyze Oracle DB object columns used in relations with proximity-based weighting.
...
class RelationalColumnsAnalyzer:
    """Analyzes Oracle database object columns used in relations with proximity weighting."""
    ...
    def analyze_object_relational_columns(self, object_name: str, top_n: int = 5) -> Dict[str, Any]:
        """Analyze relational column usage for a specific object."""
        print(f"\nAnalyzing relational columns for: {object_name}")
        ...
        for doc in documents:
            ...
            doc_column_scores = self.extract_relational_column_mentions(
                object_name, columns, context_before, line_text, context_after
            )
    ...
    def analyze_all_objects(self, top_n: int = 5, output_file: str = None) -> Dict[str, Any]:
        """Analyze relational columns for all objects."""
        print("Starting relational column analysis for all objects...")
    ...
    def _save_results(self, results: Dict[str, Any], summary: Dict[str, Any], 
        ...
        output_data = {
            'analysis_type': 'relational_columns_analysis',
            'weighting_scheme': 'line_text=3x, context_after=3x, context_before=1x',
            'proximity_algorithm': 'exponential_decay(-distance/100)',
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print("\n" + "="*80)
        print("RELATIONAL COLUMNS ANALYSIS RESULTS")
        print("="*80)
        ...
        for obj_name, obj_result in detailed_results.items():
            ...
            if top_cols_key:
                ...
                if top_columns:
                    print(f"  Top relational columns:")
...
def main():
    """Main function to run the relational columns analysis."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Analyze columns used in relations for Oracle DB objects')
    ...
    # Initialize analyzer
    analyzer = RelationalColumnsAnalyzer(solr_url=args.solr_url)
    ...
    if args.object:
        ...
        print(f"Analyzing relational columns for: {args.object}")
        result = analyzer.analyze_object_relational_columns(args.object.upper(), args.top_n)
    ...
    print("\nRelational columns analysis completed!")
